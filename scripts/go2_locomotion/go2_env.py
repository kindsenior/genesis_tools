...
from genesis.utils.geom import quat_to_xyz, transform_by_quat, inv_quat, transform_quat_by_quat, euler_to_quat
...
class Go2Env:
    ...
    def _resample_commands(self, envs_idx):
        ...
        if envs_idx is None:
            ...
            self.commands[:, 0] = torch.full((self.num_envs,), self.command_cfg["lin_vel_x_range"][1], device=gs.device)
            self.commands[:, 1] = torch.zeros((self.num_envs,), device=gs.device)
            self.commands[:, 2] = torch.zeros((self.num_envs,), device=gs.device)
