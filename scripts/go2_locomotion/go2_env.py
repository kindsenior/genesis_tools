...
class Go2Env:
    ...
    def _resample_commands(self, envs_idx):
        ...
        # set constant commands for single environment
        if envs_idx is None:
            # set constant velocity
            self.commands[:, 0] = torch.full(
                (self.num_envs,),
                self.command_cfg["lin_vel_x_range"][1],
                dtype=gs.tc_float,
                device=self.device,
            )
            self.commands[:, 1] = torch.zeros((self.num_envs,), dtype=gs.tc_float, device=self.device)
            self.commands[:, 2] = torch.zeros((self.num_envs,), dtype=gs.tc_float, device=self.device)
    ...
    def _reset_idx(self, envs_idx=None):
        ...
        if self.use_terrain: # set yaw direction randomly for terrain env
            yaw_value = torch.rand((), device=self.device) * 2 * math.pi
            reset_qpos[3:7] = torch.stack(
                (
                    torch.cos(yaw_value / 2),
                    torch.zeros((), device=self.device, dtype=gs.tc_float),
                    torch.zeros((), device=self.device, dtype=gs.tc_float),
                    torch.sin(yaw_value / 2),
                )
            )
